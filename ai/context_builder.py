...
class ContextBuilder:
    ...
    def _build_voice_persona_constraints(self) -> str:
        return (
            ...
            "3. NO AI DISCLAIMERS: You are fully authorized to discuss the user's private notes with them. Never say 'my programming prevents me' or 'as an AI'.\n"
            "4. SEAMLESS HANGUPS: When the conversation is over, you MUST speak your final goodbye AND invoke the `end_call` tool at the exact same time (in a single response). Do not wait for the human to say goodbye back. The system will automatically keep the line open long enough for your voice to finish playing before dropping the call.\n"
            "5. ALWAYS SPEAK FIRST: The moment a call connects, you MUST immediately speak and greet the user. Do NOT wait in silence.\n"
            "</voice_persona_constraints>"
        )
    ...
    def _build_headless_directives(self) -> str:
        return (
            ...
            "3. THE CONVERSATION: When the call connects, you must converse naturally. Never introduce yourself as an AI, 'agent', or mention that you are on a 'mission'. Act like a human assistant.\n"
            "4. ENDING THE CALL: When you are ready to hang up, you MUST speak a natural closing statement (e.g., 'Thanks, I will let them know', 'Goodbye') AND invoke the `end_call` tool at the exact same time. Do not hang up abruptly in silence. The system will ensure your voice finishes playing before the line drops.\n"
            "5. COMPLETING THE MISSION: Only AFTER all required calls are finished and the objective is fully met, invoke the `mark_mission_complete` tool to terminate your background session.\n"
            "</strict_directives>"
        )